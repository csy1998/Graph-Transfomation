...
def heuristic(nodeChangeNum, edgeChangeNum, score, score2):
	return score2
	return math.sqrt(nodeChangeNum*nodeChangeNum + edgeChangeNum*edgeChangeNum) + score
	# return math.sqrt(nodeChangeNum*nodeChangeNum + edgeChangeNum*edgeChangeNum) + score + 0.1*score1
		...
		for rule in rules:
			...
			for map in mappingList:
				...
				if newGraph not in costSoFar or newCost < costSoFar[newGraph]:
					...
					h=heuristic(nodeChangeNum, edgeChangeNum, score2, score1)
					priority = newCost + h
...
if __name__ == '__main__':
	...
	# 八数码问题
	# print("----------------------八数码问题----------------------")
	# problem = Input.instanceInput("examples//puzzle//instances//trivial2.json")
